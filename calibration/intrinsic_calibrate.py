"""
calibration/intrinsic_calibrate.py
==================================
...
Usage examples
--------------
  # Step 1 — capture frames (Mac built-in camera, id=0)
  python calibration/intrinsic_calibrate.py --mode capture --camera-id 0

  # Step 2 — calibrate from saved frames
  python calibration/intrinsic_calibrate.py --mode calibrate --camera-id 0

  # Step 3 — verify undistortion live
  python calibration/intrinsic_calibrate.py --mode verify --camera-id 0

  # All three steps in one go
  python camera/single_camera_calibrate.py --mode all --camera-id 0
...
------------
  ...
  (opencv-contrib is required for aruco module)
"""
...
log = logging.getLogger("single_cam_calib")
...
def make_board(squares_x: int, squares_y: int, square_size: float,
    ...
    # Tighten corner refinement for sub-pixel accuracy
...
class CoverageTracker:
    ...
    def update(self, rvec: np.ndarray, tvec: np.ndarray,
        ...
        """
        Register one captured frame.
        Returns a dict of extracted metrics for logging.
        """
    ...
    def score(self) -> float:
        """0–100 coverage score (composite across all bins)."""
    ...
    def next_suggestion(self) -> str:
        """Human-readable instruction for the next pose."""
        region_names = [
            "top-left",
            "top-center",
            "top-right",
        ...
        if self.tilt[0] < self.TARGET_PER_BIN or self.tilt[
                3] < self.TARGET_PER_BIN:
            return "Tilt board UP or DOWN sharply (+/- 25 deg)"
        if self.pan[0] < self.TARGET_PER_BIN or self.pan[
                4] < self.TARGET_PER_BIN:
            return "Rotate board LEFT or RIGHT (+/- 35 deg)"
        if self.roll[0] < self.TARGET_PER_BIN or self.roll[
                4] < self.TARGET_PER_BIN:
            return "Roll board clockwise or counter-clockwise (+/- 25 deg)"
        return "Good diversity — keep adding varied poses"

    def summary_lines(self) -> list[str]:
        """Short summary for console / overlay."""
        return [
...
def draw_coverage_hud(img: np.ndarray,
    ...
    """
    Overlay a compact coverage HUD on the frame.
    Drawn in a semi-transparent dark panel on the left side.
    """
    h, w = img.shape[:2]
    overlay = img.copy()
    ...
    _rot_row("Pan ", tracker.pan[:4])  # show 4 of 5 to fit width
    _rot_row("Roll", tracker.roll[:4])
    y += 4

    # ── Next suggestion ──
    suggestion = tracker.next_suggestion()
    # Word-wrap manually at ~36 chars
    ...
    if last_metrics:
        y += 6
        _text(
            img, f"dist={last_metrics['dist']:.2f}m  "
            f"tilt={last_metrics['tilt']:.0f}  "
            f"pan={last_metrics['pan']:.0f}  "
            f"roll={last_metrics['roll']:.0f}", (10, y), 0.40, (180, 180, 180))
...
def draw_detection_overlay(img: np.ndarray,
                           charuco_corners,
                           charuco_ids,
                           marker_corners,
                           board,
    ...
    """Draw detected ArUco markers, ChArUco corners, and pose axes."""
    if marker_corners:
    ...
    if rvec is not None and tvec is not None and K is not None:
        # Draw 3D axis — length = one square size
...
@dataclass
class CalibFrame:
    """All ChArUco data extracted from one frame."""
    charuco_corners: np.ndarray  # (N,1,2) float32
    charuco_ids: np.ndarray  # (N,1)   int32
    img_size: tuple  # (w, h)
    frame_path: Path
    metrics: dict  # dist, tilt, pan, roll
...
def run_capture(args) -> None:
    """
    Live capture session with coverage HUD.
    ...
    if not cap.isOpened():
        log.error("Cannot open camera index %d", args.camera_index)
        sys.exit(1)
    ...
    K_approx = None  # rough K for pose axes (updated each frame)
    D_approx = np.zeros(5)
    ...
    while True:
        ...
        # Estimate pose if we have at least a rough K
        rvec, tvec = None, None
        if valid_detection and K_approx is not None:
            ok_pose, rvec, tvec = aruco.estimatePoseCharucoBoard(
                charuco_corners,
                charuco_ids,
                board,
                K_approx,
        ...
        # Build rough K from image size (good enough for HUD pose display)
        if K_approx is None:
            f = max(w, h)
        ...
        # Detection readiness indicator (top-right)
        if valid_detection:
        ...
        cv2.imshow(f"Intrinsic calibration — cam {args.camera_id}", disp)
        ...
        # ── C: calibrate now ──────────────────
            ...
            if tracker.n_captured < 5:
                ...
                _run_calibration_from_dir(out_dir, board, charuco_detector,
                                          args)
                return
    ...
    if tracker.is_sufficient():
        ...
        log.warning(
            "Coverage score %.0f%% < 75%% — consider capturing more poses",
            tracker.score(),
        )
...
def _run_calibration_from_dir(frame_dir: Path, board, charuco_detector,
                              args) -> Optional[Path]:
    """
    Read all PNG frames in frame_dir, detect ChArUco, run calibration.
    Returns path to saved .npz, or None on failure.
    """
    ...
    if good_count < 10:
        log.error(
            "Only %d usable frames (need >= 10). Capture more varied poses.",
            good_count,
        )
        return None

    log.info("Running cv2.aruco.calibrateCameraCharuco on %d frames…",
             good_count)

    rms, K, D, rvecs, tvecs = aruco.calibrateCameraCharuco(
    ...
    if rms > 1.0:
        ...
        log.warning("RMS %.4f px is acceptable but not ideal (target < 0.5)",
                    rms)
    else:
    ...
    if per_frame_errors:
        ...
        log.info(
            "Per-frame reprojection error — "
            "min=%.3f  mean=%.3f  max=%.3f  std=%.3f", min(errors),
            np.mean(errors), max(errors), np.std(errors))
        bad = [(i, e) for i, e in per_frame_errors if e > rms * 2.5]
        if bad:
            log.warning(
                "High-error frames (consider removing and recalibrating):")
            for i, e in bad:
    ...
    # Also save human-readable JSON alongside
...
def run_verify(args) -> None:
    ...
    if not npz_path.exists():
        log.error("Intrinsics not found at %s — run --mode calibrate first",
                  npz_path)
        sys.exit(1)
    ...
    # Precompute undistortion map (faster than remap each frame)
    new_K, roi = cv2.getOptimalNewCameraMatrix(K,
                                               D,
                                               img_size,
                                               alpha=0,
                                               newImgSize=img_size)
    ...
    cap = cv2.VideoCapture(args.camera_index)
    if not cap.isOpened():
        log.error("Cannot open camera %d", args.camera_index)
        sys.exit(1)
    ...
    while True:
        ...
        cv2.imshow(f"Verify intrinsics — cam {args.camera_id}", combined)
        key = cv2.waitKey(1) & 0xFF
        if key == ord("q"):
            break
        if key == ord("s"):
            ...
            cv2.imwrite(str(snap_dir / f"orig_{snap_count:03d}_{ts}.png"),
                        frame)
            cv2.imwrite(str(snap_dir / f"undist_{snap_count:03d}_{ts}.png"),
                        undist)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Single-camera intrinsic calibration (ChArUco)",
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Logical camera ID (0=left, 1=right). Used for output filenames.")
    p.add_argument(
        "--camera-index",
        type=int,
        default=0,
        help="OpenCV VideoCapture index (usually same as camera-id on Mac)")
    ...
    # ChArUco board — MUST match your Android app
    p.add_argument("--squares-x",
                   type=int,
                   default=6,
                   help="Chessboard squares horizontally")
    p.add_argument("--squares-y",
                   type=int,
                   default=5,
                   help="Chessboard squares vertically")
    p.add_argument("--square-size",
                   type=float,
                   default=0.04,
                   help="Physical square size in metres")
    p.add_argument("--marker-size",
                   type=float,
                   default=0.03,
                   help="Physical ArUco marker size in metres")
    p.add_argument("--aruco-dict",
                   type=str,
                   default="DICT_4X4_50",
                   choices=list(ARUCO_DICTS),
                   help="ArUco dictionary — must match your Android app")

    # Calibration options
    p.add_argument("--min-corners",
                   type=int,
                   default=6,
                   help="Minimum ChArUco corners required per frame")
    p.add_argument("--min-frames",
                   type=int,
                   default=20,
...
def main() -> None:
    ...
    log.info("Mode=%s  camera_id=%d  camera_index=%d", args.mode,
             args.camera_id, args.camera_index)
    log.info("Board: %dx%d squares  square=%.4fm  marker=%.4fm  dict=%s",
             args.squares_x, args.squares_y, args.square_size,
             args.marker_size, args.aruco_dict)
